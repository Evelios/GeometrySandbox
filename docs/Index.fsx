...
let canvas = Canvas.a4 Orientation.Landscape Length.inch
...
let square: BoundingBox2D<Meters, Cartesian> =
    BoundingBox2D.from (Point2D.xy (width / 2.) (height / 2.)) (Point2D.xy (width * 2. / 3.) (height * 2. / 3.))
...
let red = Pen.withThickness (Length.millimeters 0.7) "#FF0000"

let green = Pen.withThickness (Length.millimeters 0.5) "#00FF00"

let blue = Pen.withThickness (Length.millimeters 0.3) "#0000FF"
...
let plotRed =
    Plotter.create canvas [ Layer.withPen red [ circle; square; triangle ] ]
