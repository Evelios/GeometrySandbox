module Math.Noise.GradientNoise
    ...
    let fractalBrownianMotion octaves lacunarity hz0 gain gain0 (fn: float -> float -> float) =
        let rec fbm hz amp octaves x y =
            if octaves = 0 then
                ...
                (fn (x * hz) (y * hz)) * amp
                + fbm (hz * lacunarity) (amp * gain) (octaves - 1) x y
...
module GradientNoise =
    ...
    let makeGradNoise (seed: int) gridSize roughness =
        ...
        let grads = [| for _ in 0..maxPerms -> normalise (guass (), guass ()) |]
        let gradsx, gradsy = [| for x, _ in grads -> x |], [| for _, y in grads -> y |]
